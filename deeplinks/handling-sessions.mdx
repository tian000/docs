...
Sessions do not expire. Once a user has connected with Phantom, the corresponding app can indefinitely make requests such as [SignAndSendTransaction](provider-methods/signandsendtransaction) and [SignMessage](provider-methods/signmessage) without prompting the user to re-connect with Phantom. Apps will still need to re-connect to Phantom after a [Disconnect](provider-methods/disconnect) event or an [Invalid Session](handling-sessions.md#invalid-sessions).
...
* **JSON Data**: A JSON object with the following fields:
  * `app_url` (string): A url used to fetch app metadata (i.e. title, icon) using the same properties found in [Displaying Your App](/best-practices/displaying-your-app).
...
2. It was signed by the current wallet keypair, but the session's JSON `data` does not pass muster. There are a few reasons why this might occur:
   ...
   2. The `app_url` could be blocked if malicious. See [Blocklist](/development-powertools/blocklist) for more information.
