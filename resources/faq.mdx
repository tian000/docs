...
Phantom will only [inject its provider](/getting-started-with-solana/detecting-the-provider) into websites that begin with `https://`, or if the host is `localhost` or `127.0.0.1`. If your website only uses `http://`, Phantom will not inject its provider and you will not be able to access the methods found at `window.phantom`. Encrypting your web traffic and upgrading to `https://` will restore functionality.
...
[supported-media-types.md](/best-practices/tokens/supported-media-types)
{% endcontent-ref %}
