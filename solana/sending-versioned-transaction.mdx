...
const lookupSignature = await provider.signAndSendTransaction(lookupTransaction);</code></pre>
...
const extensionSignature = await provider.signAndSendTransaction(extensionTransactionV0);
```
